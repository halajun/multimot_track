     ...
     mTimeStamp(timeStamp), mK(K.clone()),mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth)
{
    ...
    for (int i = 0; i < mvKeys.size(); ++i)
    {
        ...
        // int x = test_p.pt.x;
        // int y = test_p.pt.y;
        // float flow_x_t = imFlow.at<cv::Vec2f>(y,x)[0];
        // float flow_y_t = imFlow.at<cv::Vec2f>(y,x)[1];
        // cout << "flow verify: " << x + flow_x_t << " "  << y + flow_y_t << endl;
        ...
        //     int y = mvKeys[i].pt.y;
        //     float flow_x, flow_y;
        //     if(imFlow.at<cv::Vec2f>(y,x)[0]!=0 && imFlow.at<cv::Vec2f>(y,x)[1]!=0){
        //         // cout << "flow before: " << imFlow.at<cv::Vec2f>(y,x)[0] << " " << imFlow.at<cv::Vec2f>(y,x)[1] << endl;
        //         flow_x = imFlow.at<cv::Vec2f>(y,x)[0] + rng.gaussian(4.0);
        //         flow_y = imFlow.at<cv::Vec2f>(y,x)[1] + rng.gaussian(4.0);
        //         // cout << "flow after: " << flow_x << " " << flow_y << endl;
        //     }
        //     else{
        //         flow_x = imFlow.at<cv::Vec2f>(y,x)[0];
        //         flow_y = imFlow.at<cv::Vec2f>(y,x)[1];
        //     }
        //     if(flow_x!=0 && flow_y!=0)
        //     {
        //         if(mvKeys[i].pt.x+flow_x < imGray.cols-1 && mvKeys[i].pt.y+flow_y < imGray.rows-1 && mvKeys[i].pt.x < imGray.cols-1 && mvKeys[i].pt.y < imGray.rows-1){
        //             mvSiftKeysTmp.push_back(mvKeys[i]);
        //             mvCorres.push_back(cv::KeyPoint::KeyPoint(mvKeys[i].pt.x+flow_x,mvKeys[i].pt.y+flow_y,0,0,0,-1));
        //             vCorSta.push_back(-1);
        //             fal_ma = fal_ma + 1;
        //         }
        //     }
    ...
    // ---------------------------------------------------------------------------------------
    // ++++++++++++++++++++++++++++ New added for sift features ++++++++++++++++++++++++++++
    // ---------------------------------------------------------------------------------------

    //     // ******************* CUDA sift extraction *********************

    //     SiftData siftData;
    //     InitSiftData(siftData, 2099, true, true);

    //     cv::Mat limg;
    //     imGray.convertTo(limg, CV_32FC1);
    ...
    //     CudaImage img;
    //     img.Allocate(1242, 375, 1242, false, NULL, (float*) limg.data);
    ...
    //     float thresh = 3.0f;   /* Threshold on difference of Gaussians for feature pruning */
    ...
    //     N_s = siftData.numPts; // number of sift feature
    //     mvSiftKeys.resize(N_s);
    //     mSift = cv::Mat(N_s, 128, CV_32F);
    //     SiftPoint *sift = siftData.h_data;
    //     cv::KeyPoint kp_tmp;
    //     for (int i = 0; i < N_s; ++i)
    //     {
    //         // descriptor
    //         float *data = sift[i].data;
    //         cv::Mat sift_tmp = cv::Mat(1, 128, CV_32F, data);
    //         sift_tmp.copyTo(mSift.row(i));
    //         // keypoint
    //         mvSiftKeys[i] = cv::KeyPoint::KeyPoint(sift[i].xpos,sift[i].ypos,sift[i].scale,sift[i].orientation,0,-1);
    //     }
    ...
    //     // ******* Compute the near neighbor indexes for each feature ********

    //     // initialize variables
    //     vnNeigh.resize(N_s);
    //     vnNeighDist.resize(N_s);
    //     cv::Mat_<float> features(0,2);
    //     int top_n = 20;
    //     cv::Mat indices = cv::Mat(1, top_n, CV_32SC1);
    //     cv::Mat dists = cv::Mat(1, top_n, CV_32F);
    //     for (int i = 0; i < N_s; ++i){
    //         cv::Mat row = (cv::Mat_<float>(1, 2) << mvSiftKeys[i].pt.x,mvSiftKeys[i].pt.y);
    //         features.push_back(row);
    //     }
    //     // construct flann function
    //     cv::flann::GenericIndex<cvflann::L2<float> > index(features, cvflann::KMeansIndexParams());
    //     // loop for each feature
    //     for (int i = 0; i < N_s; ++i)
    //     {
    //         cv::Mat query = (cv::Mat_<float>(1, 2) << mvSiftKeys[i].pt.x,mvSiftKeys[i].pt.y);
    //         // search knn
    //         index.knnSearch(query, indices, dists, top_n, cvflann::SearchParams());
    //         int count = 0;
    //         for (int j = 0; j < top_n; ++j)
    //         {
    //             // save knn
    //             if(dists.at<float>(0,j)>0){
    //                 vnNeigh[i].push_back(indices.at<int>(0,j));
    //                 vnNeighDist[i].push_back(std::sqrt(dists.at<float>(0,j)));
    //                 count = count + 1;
    //                 // cout << std::sqrt(dists.at<float>(0,j)) << " ";
    //             }
    //             if( count==10 || std::sqrt(dists.at<float>(0,j))>20.0 )
    //                 break;
    //         }
    //         // cout << endl;
    //     }
    ...
    //     cout << "time usage for flann: "  << ttrack << endl;

    //     // ******************* (Get Depth) ******************************************

    //     // assign the depth value to each sift keypoint
    //     mvSiftDepth = vector<float>(N_s,-1);
    //     for(int i=0; i<N_s; i++)
    //     {
    //         const cv::KeyPoint &kp = mvSiftKeys[i];
    ...
    //     // ******************* (New added for semantic mask) ********************************

    //     // Assign the semantic label to each extracted feature
    //     vSemLabel.resize(N_s,-1);
    //     vObjLabel_gt.resize(N_s,-1);
    //     int u, v;
    //     // cout << maskSEM.cols << " " << maskSEM.rows << endl;
    //     for (int i = 0; i < N_s; ++i)
    //     {
    ...
    //         vSemLabel[i] = maskSEM.at<int>(v,u);  // be careful with the order  !!!
    //     }
    //     // cout << endl;
    ...
    // cv::Mat img_show;
    // cv::drawKeypoints(imGray, mvSiftKeys, img_show, cv::Scalar::all(-1), cv::DrawMatchesFlags::DEFAULT); // cv::DrawMatchesFlags::DRAW_RICH_KEYPOINTS
    // cv::imshow("SIFT Detection", img_show);
    // cv::waitKey(0);
